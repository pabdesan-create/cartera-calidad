...
8. ERROR FRECUENTE A EVITAR: coger por descuido la columna INMEDIATAMENTE ANTERIOR al año base (un año más antigua). Si dudas entre dos columnas adyacentes, la correcta es SIEMPRE la más a la derecha de las dos que tenga fecha real en Report Date, nunca la de la izquierda
...
══════════════════════════════════════════════
SI se te proporciona el panel "Dividend Growth" de Koyfin (Consecutive Annual Increases + Annualized Growth % a 1Y/3Y/5Y/10Y):
- ÚSALO COMO FUENTE PRINCIPAL, no calcules el CAGR a mano desde la fila de DPS
- dgi_rachaAnios = el valor exacto de "Consecutive Annual Increases"
- dgi_cagrDiv = el "Annualized Growth %" a 5Y de ese panel (si no hay 5Y, usa el periodo más largo disponible: 10Y > 3Y > 1Y)
- Indica en dgi_notas qué periodo usaste (ej: "CAGR Div 5Y según panel Dividend Growth de Koyfin")
- PAGADOR IRREGULAR (dividendos especiales puntuales, algún año sin pago, recorte pasado): si "Consecutive Annual Increases" = 0 o es muy bajo pero la empresa SÍ paga dividendo de forma habitual, NO fuerces una racha inexistente. Usa igualmente el "Annualized Growth %" del periodo disponible (aunque la racha sea 0), y explica la irregularidad en dgi_notasMacro (ej: "Recortó el dividendo en 2020, racha reiniciada desde entonces" o "Paga dividendos especiales variables además del ordinario, lo que distorsiona el cálculo de racha"). El campo dgi_yieldVsHistorico y el resto de campos siguen calculándose con normalidad

SI NO se te proporciona ese panel, calcula desde la fila DPS de Per Share Data:
...
- Si la empresa es un PAGADOR IRREGULAR (algún año sin dividendo o recorte visible en la fila DPS), pon racha=0 (o los años desde el último recorte) y explica la irregularidad en dgi_notasMacro en vez de forzar un cálculo engañoso
...
const SCREENSHOTS_INFO=[{n:1,label:'Actuals Superior',hint:'Income Statement + Balance Sheet + Cash Flow'},{n:2,label:'Actuals Inferior',hint:'Per Share (DPS, EPS) + Márgenes + Growth Rates'},{n:3,label:'Returns',hint:'ROIC, ROE, Div Yield, Buyback Yield, Shares Outstanding'},{n:4,label:'Dividend Growth (opcional)',hint:'Solo si paga dividendo de forma regular. Recorta ajustado al panel (coste extra mínimo). Si el pago es irregular, omítela — la app lo detecta igualmente desde la fila DPS'}]
...
// DGI SCORING ENGINE
// ═══════════════════════════════════════════════════════════════
const DS={chowder:v=>v>=16?10:v>=12?7:v>=10?4:0,cagrDiv:v=>v>=15?10:v>=10?7:v>=7?4:1,racha:v=>v>=25?10:v>=15?7:v>=10?5:v>=7?3:v>=4?1:0,payFCF:v=>v<40?12:v<55?9:v<70?5:0,cagrFCF:v=>v>=15?10:v>=10?7:v>=7?4:1,cagrBPA:v=>v>=15?8:v>=10?6:v>=7?3:1,roic:v=>v>=20?4:v>=15?3:v>=12?2:0,moatW:v=>v==="amplio"?6:v==="estrecho"?3:0,moatT:v=>({monopolio_duopolio:7,red_clientes:6,costes_cambio:5,datos_propietarios:4,escala_marca:2,ninguna:0}[v]||0),deuda:v=>v<1.5?5:v<2.5?3:v<3.5?1:0,rating:v=>["AAA","AA+","AA","AA-","A+","A","A-"].includes(v)?3:v==="BBB+"?2:v==="BBB"?1:0,margenTend:v=>v==='expansion_apalancamiento'?5:v==='expansion'?3:v==='estable'?2:0,yldH:v=>v==="mayor"?6:v==="igual"?3:0,perH:v=>v==="bajo"?4:v==="en_linea"?2:0}
function dgiCalcScore(c){
  const y=+c.yieldActual||0,d=+c.cagrDiv5Y||0,ch=y+d
  // BLOQUE A (30 pts): Chowder + CAGR Div + Racha — Yield eliminado (ya capturado en Chowder)
  const A=DS.chowder(ch)+DS.cagrDiv(d)+DS.racha(+c.rachaAnios||0)
  // BLOQUE B (30 pts): Payout FCF + CAGR FCF + CAGR BPA (sustituye payEPS)
  ...
  const B=payFCFPts+DS.cagrFCF(f5)+DS.cagrBPA(+c.crecBPA5Y||0)
  // BLOQUE C (30 pts): ROIC + Moat (anchura+tipo) + Deuda + Rating + Tendencia Márgenes
  const C2=DS.roic(+c.roic||0)+DS.moatW(c.moat||"ninguno")+DS.moatT(c.tipoMoat||"ninguna")+DS.deuda(+c.deudaEbitda||0)+DS.rating(c.rating||"Sin rating")+DS.margenTend(c.margenTendencia||'estable')
...
function DcfHistorial({historia,numCols}){
  if(!historia||historia.length<2)return<td colSpan={numCols} style={{padding:'8px 16px',fontSize:11,color:'#94a3b8',fontStyle:'italic'}}>Solo 1 análisis guardado — vuelve a guardar para ver evolución.</td>
  const h=[...historia].sort((a,b)=>a.fecha.localeCompare(b.fecha))
  const fmtF=f=>{const d=new Date(f+'T12:00');return`${d.toLocaleString('es-ES',{month:'short'})}'${String(d.getFullYear()).slice(2)}`}
  const arr=(v,p)=>{if(v==null||p==null)return'';const d=v-p;return d>0?<span style={{color:'#15803d',fontSize:9,marginLeft:2}}>▲</span>:d<0?<span style={{color:'#dc2626',fontSize:9,marginLeft:2}}>▼</span>:''}
  const rows=[
    {l:'Precio',f:(v,p)=><span>{v}{arr(v,p)}</span>,k:'price'},
    {l:'PO Media',f:(v,p)=><span style={{fontWeight:700}}>{v!=null?v.toFixed(0):'—'}{arr(v,p)}</span>,k:'poMed'},
    {l:'PO BN',f:(v,p)=><span>{v!=null?v.toFixed(0):'—'}{arr(v,p)}</span>,k:'poBn'},
    {l:'PO FCF',f:(v,p)=><span>{v!=null?v.toFixed(0):'—'}{arr(v,p)}</span>,k:'poFcf'},
    {l:'Dif% Media',f:(v,p)=><span style={{color:v>0?'#15803d':v<-15?'#dc2626':'#b45309',fontWeight:700}}>{v!=null?(v>=0?'+':'')+v.toFixed(1)+'%':'—'}{arr(v,p)}</span>,k:'difMed'},
    {l:'Dif% BN',f:(v,p)=><span>{v!=null?(v>=0?'+':'')+v.toFixed(1)+'%':'—'}{arr(v,p)}</span>,k:'difBn'},
    {l:'CAGR BN',f:(v,p)=><span>{v!=null?v+'%':'—'}{arr(v,p)}</span>,k:'cagrBn'},
    {l:'CAGR FCF',f:(v,p)=><span>{v!=null?v+'%':'—'}{arr(v,p)}</span>,k:'cagrFcf'},
  ]
  return(
    <td colSpan={numCols} style={{padding:'12px 16px',background:'#0a1628'}}>
      ...
      <table style={{borderCollapse:'collapse',fontSize:11}}>
        <thead>
          <tr>
            <th style={{textAlign:'left',color:'#64748b',fontWeight:600,padding:'2px 16px 6px 0',width:80}}>Métrica</th>
            {h.map((s,i)=><th key={i} style={{color:'#94a3b8',fontWeight:700,padding:'2px 16px 6px',textAlign:'right',background:i===h.length-1?'#0f2246':'transparent',borderRadius:i===h.length-1?'6px 6px 0 0':0}}>{fmtF(s.fecha)}{i===h.length-1&&<div style={{fontSize:8,color:'#3b82f6'}}>actual</div>}</th>)}
          </tr>
        </thead>
        <tbody>
          {rows.map(({l,f,k})=>(
            <tr key={l} style={{borderTop:'1px solid #1e2a3b'}}>
              <td style={{color:'#64748b',padding:'4px 16px 4px 0',fontWeight:600}}>{l}</td>
              {h.map((s,i)=>{const prev=i>0?h[i-1][k]:undefined;return<td key={i} style={{textAlign:'right',padding:'4px 16px',background:i===h.length-1?'#0f2246':'transparent',color:'#cbd5e1',whiteSpace:'nowrap'}}>{f(s[k],prev)}</td>})}
            </tr>
          ))}
        </tbody>
      </table>
...
function DgiHistorial({historia}){
  ...
  const rows=[
    {l:'Score',f:(v,p)=>num(v,p,'/90')},
    {l:'Clasif.',f:(v,p)=>cat(v,p),key:'clasificacion'},
    {l:'Yield',f:(v,p)=>num(v,p,'%'),key:'yieldActual'},
    {l:'CAGR Div',f:(v,p)=>num(v,p,'%'),key:'cagrDiv5Y'},
    {l:'Racha',f:(v,p)=>num(v,p,'a'),key:'rachaAnios'},
    {l:'Pay.FCF',f:(v,p)=>num(v,p,'%'),key:'payoutFCF'},
    {l:'CAGR FCF',f:(v,p)=>num(v,p,'%'),key:'cagrFCF5Y'},
    {l:'CAGR BPA',f:(v,p)=>num(v,p,'%'),key:'crecBPA5Y'},
    {l:'ROIC',f:(v,p)=>num(v,p,'%'),key:'roic'},
    {l:'Deuda',f:(v,p)=>num(v,p,'x'),key:'deudaEbitda'},
    {l:'Rating',f:(v,p)=>cat(v,p),key:'rating'},
    {l:'Márgenes',f:(v,p)=>cat((v||'').replace(/_/g,' '),(p||'').replace(/_/g,' ')),key:'margenTendencia'},
  ]
  return(
    <div style={{background:'#f8fafc',border:'1px solid #e2e8f0',borderRadius:10,padding:'12px 14px',marginBottom:10}}>
      ...
      <div style={{overflowX:'auto'}}>
        <table style={{borderCollapse:'collapse',width:'100%',fontSize:11}}>
          <thead>
            <tr>
              <th style={{textAlign:'left',color:'#94a3b8',fontWeight:600,padding:'2px 10px 6px 0',width:70}}>Métrica</th>
              {h.map((s,i)=><th key={i} style={{color:'#475569',fontWeight:700,padding:'2px 10px 6px',textAlign:'center',background:i===h.length-1?'#eff6ff':'transparent',borderRadius:i===h.length-1?'6px 6px 0 0':0}}>{fmtFecha(s.fecha)}{i===h.length-1?<div style={{fontSize:9,color:'#3b82f6',fontWeight:400}}>actual</div>:null}</th>)}
            </tr>
          </thead>
          <tbody>
            {rows.map(({l,f,key})=>{
              const fld=key||l.toLowerCase()
              return(
                <tr key={l} style={{borderTop:'1px solid #f1f5f9'}}>
                  <td style={{color:'#94a3b8',padding:'4px 10px 4px 0',fontWeight:600}}>{l}</td>
                  {h.map((s,i)=>{
                    const val=fld==='Score'?s.score:s[fld]
                    const prev=i>0?(fld==='Score'?h[i-1].score:h[i-1][fld]):undefined
                    return<td key={i} style={{textAlign:'center',padding:'4px 10px',background:i===h.length-1?'#eff6ff':'transparent',color:'#334155',whiteSpace:'nowrap'}}>{f(val,prev)}</td>
                  })}
                </tr>
              )
            })}
...
function QualityHistorial({historia}){
  ...
  const rows=[
    ...
    {l:'CAGR FCF',f:(v,p)=>num(v,p,'%'),key:'cagrFCF_historico'},
  ...
  return(<div style={{background:'white',borderRadius:12,overflow:'hidden',border:`1px solid ${expanded?cl.border:'#e2e8f0'}`,boxShadow:expanded?`0 0 0 2px ${cl.border}`:'0 1px 3px rgba(0,0,0,.04)',transition:'all .15s'}}>
    <div onClick={onToggle} style={{padding:'14px 16px',display:'flex',alignItems:'center',gap:12,cursor:'pointer',flexWrap:'wrap'}}>
      ...
      <div style={{display:'flex',gap:6,flexWrap:'wrap',alignItems:'center'}}>
        ...
        <QPill label={co.nAniosHistorico?`CAGR ${co.nAniosHistorico}A`:"CAGR"} value={`${co.crecimientoCAGR}%`} good={parseFloat(co.crecimientoCAGR)>=10}/>
        {co.cagrFCF_historico&&<QPill label={co.nAniosHistorico?`CAGR FCF ${co.nAniosHistorico}A`:"CAGR FCF"} value={`${co.cagrFCF_historico}%`} good={parseFloat(co.cagrFCF_historico)>=10}/>}
    ...
    {expanded&&(<div style={{background:'#fafafa',padding:16,borderTop:`1px solid ${cl.border}`}}>
      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(180px,1fr))',gap:16,marginBottom:12}}>
        <div>
          ...
          {[['Margen Neto',`${co.margenNeto}%`,parseFloat(co.margenNeto)>=20],['ROIC',`${co.roic}%`,parseFloat(co.roic)>=12],[co.nAniosHistorico?`CAGR (${co.nAniosHistorico} años)`:'Crec. CAGR',`${co.crecimientoCAGR}%${co.anioInicio&&co.anioBase?` · ${co.anioInicio}→${co.anioBase}`:''}`,parseFloat(co.crecimientoCAGR)>=10],...(co.cagrFCF_historico?[[co.nAniosHistorico?`CAGR FCF (${co.nAniosHistorico} años)`:'CAGR FCF',`${co.cagrFCF_historico}%`,parseFloat(co.cagrFCF_historico)>=10]]:[]),['D/EBITDA',`${co.deudaEbitda}x`,parseFloat(co.deudaEbitda)<=2],['FCF Margin',`${co.fcfMargin}%`,parseFloat(co.fcfMargin)>=20],['P/E Forward',`${co.peForward}x`,null],['Market Cap',co.marketCap||'—',null]].map(([l,v,ok])=><KV key={l} label={l} value={v} ok={ok}/>)}
        </div>
        <div>
...
function ImageDropzone({images,setImages}){
  ...
  return(<div>
    <div onDragOver={e=>{e.preventDefault();setDragging(true)}} onDragLeave={()=>setDragging(false)} onDrop={e=>{e.preventDefault();setDragging(false);addFiles(e.dataTransfer.files)}} onClick={()=>inputRef.current?.click()} style={{border:`2px dashed ${dragging?'#3b82f6':'#93c5fd'}`,borderRadius:12,padding:'24px 20px',textAlign:'center',cursor:'pointer',background:dragging?'#eff6ff':'#f8fafc',marginBottom:16}}>
      ...
      <div style={{fontWeight:700,color:'#1e293b',marginBottom:4}}>Arrastra los 3 screenshots de Koyfin (+1 opcional)</div>
      <div style={{fontSize:12,color:'#64748b'}}>o haz click · {images.length}/3 obligatorias {images.length>3&&`+ ${images.length-3} opcional(es)`}</div>
...
${ANALYSIS_PROMPT}`
  ...
  const saveQResult=()=>{
    ...
    const snapQ={fecha,score:qResult.score,clasificacion:qResult.clasificacion,margenNeto:qResult.margenNeto,
      roic:qResult.roic,crecimientoCAGR:qResult.crecimientoCAGR,cagrFCF_historico:qResult.cagrFCF_historico,deudaEbitda:qResult.deudaEbitda,
  ...
  const saveDcf=()=>{const errs={};['ticker','bn','cagrBn','mktCap','price'].forEach(k=>{if(!dcfForm[k]&&dcfForm[k]!=='0')errs[k]=true});if(Object.keys(errs).length){setDcfErrors(errs);return};setDcfErrors({})
    const row=mkRow(dcfForm)
    // Snapshot de historial DCF: precio, POs, difs, CAGRs
  ...
  return(<div style={{minHeight:'100vh',background:'#f0f4f8',fontFamily:'system-ui,-apple-system,sans-serif'}}>
    ...
    <div style={{maxWidth:1300,margin:'0 auto',padding:'20px 16px'}}>
      ...
      {tab==='dgi-analizar'&&(<div style={{display:'flex',flexDirection:'column',gap:16}}>
        ...
        {/* Bloque A */}
        <DGICard>
          <DGIBHeader letter="A" title="El Dividendo" pts={dgiSc.A} max={30}/>
        ...
        {/* Bloque C */}
        <DGICard>
          <DGIBHeader letter="C" title="Calidad del Negocio" pts={dgiSc.C} max={30}/>
          <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:12}}>
            ...
            <DGIFld label="Tendencia márgenes EBIT 10Y" hint="Variación >5pp en 10 años"><DGISel value={dgiForm.margenTendencia||'estable'} onChange={dgiUpd('margenTendencia')} options={[{v:'expansion_apalancamiento',l:'📈 Expansión con apalancamiento operativo (+5pts)'},{v:'expansion',l:'↑ En expansión >5pp (+3pts)'},{v:'estable',l:'→ Estables ±5pp (+2pts)'},{v:'declive',l:'↓ En declive >5pp (0pts)'}]}/></DGIFld>
          </div>
        </DGICard>
              ...
              {items.map(co=>{
                ...
                return(<div key={co.id} style={{background:'white',border:`1px solid ${isOpen?cm.border:'#e2e8f0'}`,borderRadius:12,overflow:'hidden',boxShadow:isOpen?`0 0 0 2px ${cm.border}`:'0 1px 3px rgba(0,0,0,.04)'}}>
                  ...
                  {isOpen&&(<div style={{background:'#fafafa',padding:16,borderTop:`1px solid ${cm.border}`}}>
                    ...
                    <DgiHistorial historia={co.historia}/>
